...
PROJECTION_FILE_FORMAT = 'raw' # Indicate we're loading a single raw file
PROJECTION_FILE_PATTERN = 'walnut.raw' # Exact filename
# Not needed for single file:
# PROJECTION_INDEX_RANGE = None

# Flat/Dark field file names (Walnut dataset often doesn't include these)
# FLAT_FIELD_AVG_FILENAME = None
# DARK_FIELD_AVG_FILENAME = None

# --- 3. Geometry and Setup Parameters ---
# !! MUST BE ADJUSTED based on walnut.dat !!
NUM_PROJECTIONS = 41664000 # From .dat file (e.g.)
ANGLE_START_DEG = 0.0
ANGLE_END_DEG = 360.0 # Often 360 for cone beam
ANGLES_DEG = np.linspace(ANGLE_START_DEG, ANGLE_END_DEG, NUM_PROJECTIONS, endpoint=False)
ANGLES_RAD = np.deg2rad(ANGLES_DEG)

# Detector Parameters from .dat file (e.g.)
DETECTOR_PIXEL_SIZE_MM = 0.4
DETECTOR_SHAPE = (100, 100) # (height, width) - CRITICAL!

GEOMETRY_TYPE = 'cone' # Should be cone beam

# Distances from .dat file (e.g.)
DISTANCE_SOURCE_DETECTOR_MM = 949.0
DISTANCE_SOURCE_OBJECT_MM = 541.0

# Center of Rotation from .dat file (e.g.)
CENTER_OF_ROTATION_OFFSET_PX = 0.0

# --- 4. Preprocessing Parameters ---
# Disable flat/dark if files not available
PERFORM_FLAT_FIELD_CORRECTION = False
PERFORM_DARK_FIELD_CORRECTION = False
# Log conversion might still be needed if the .raw data is transmission (I/I0)
# If it's already attenuation (-log(I/I0)), set this to False. Check .dat or source.
PERFORM_LOG_CONVERSION = True # Assume transmission for now

# --- 5. Reconstruction Parameters ---
# Use an algorithm suitable for cone-beam
RECONSTRUCTION_ALGORITHM = 'FDK' # Or SIRT, CGLS (requires ASTRA/TIGRE)

# Reconstruction Volume Parameters (adjust as needed)
RECON_VOXEL_SIZE_MM = DETECTOR_PIXEL_SIZE_MM # Start with this
RECON_VOLUME_SHAPE = (DETECTOR_SHAPE[0], DETECTOR_SHAPE[1], DETECTOR_SHAPE[1]) # (Z, Y, X)

# --- Add Data Type Information (NEW) ---
# Add this parameter to config.py to specify the data type in walnut.raw
# Get this from walnut.dat (e.g., 'float32', 'uint16')
RAW_DATA_TYPE = np.float32 # Example: Use np.float32, np.uint16, etc.
...
PROJECTION_DIR = os.path.join(DATA_DIR, 'projections', 'walnut') # Subdirectory for a specific scan
...
ERFORM_LOG_CONVERSION = True # Convert transmission to attenuation (-log(I/I0))
